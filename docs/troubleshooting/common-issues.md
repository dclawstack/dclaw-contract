...
# Check events
kubectl get events -n dclaw-contract --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-contract deployment/dclaw-contract-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-contract-db-credentials -n dclaw-contract
   ```
