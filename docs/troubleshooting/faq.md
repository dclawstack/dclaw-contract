...
### How do I update DClaw Contract?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp contract --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Contract without Kubernetes?

Yes, for local development:

```bash
cd dclaw-contract/frontend && npm run dev
cd dclaw-contract/backend && uvicorn dclaw_contract.main:app --reload
```
